...
**스킬**: 42개 (자동 등록 — `skills/skill_*.py` 파일만 추가하면 됨)
...
- Playwright Chromium — 웹 수집 스킬(`skill_web_collector.py`) 사용 시 최초 1회 `playwright install chromium` 필요
...
playwright install chromium   # 웹 수집 스킬 최초 1회 필요 (브라우저 바이너리 설치)
...
| **웹 수집** | "네이버 부동산에서 대전 서구 아파트 수집해줘" | Playwright 기반, 로그인·필터·페이지네이션이 필요한 사이트 전용 |
...
| 가상 키보드 | "안녕하세요라고 입력해줘", "방금 대답 입력해줘" | pyperclip+pyautogui, 현재 포커스된 창에 붙여넣기 (Enter는 "엔터"라고 말했을 때만) |
...
`ClaudeCliEngine.run_task()`는 이전 호출이 남긴 세션을 `--resume`으로 이어갈 수
있는 능력(`resume=True`)을 가지고 있습니다. 매 호출을 매번 새 세션으로 시작하지
않아도 되지만, 현재는 엔진 능력으로만 존재하고 `skill_agent.py`가 자동으로
켜지 않습니다(서로 무관한 조사 작업이 이어져 뒤섞이는 걸 막기 위함).

### 웹 수집 (Playwright, 2.0 신규)

`skill_agent.py`의 내장 `WebFetch`가 다루지 못하는 자바스크립트 렌더링·로그인·
필터·페이지네이션이 필요한 사이트(부동산, 쇼핑몰, 중고거래 등)를 대상으로
합니다. 스크린샷/UIA 대신 Playwright로 뽑은 DOM 텍스트를 Claude에 넘겨
`hybrid_screen.py`보다 스텝당 빠릅니다. 구매·결제·삭제·전송·제출처럼 되돌릴
수 없는 액션은 요소 텍스트에 그런 키워드가 보이면 최선의 노력으로 차단합니다.
로그인 세션은 `data/web_collector_state.json`에 저장해 재사용합니다. "저장해줘"/
"엑셀로" 같은 명시적 요청이 있을 때만 xlsx로 저장하고, 그 외에는 수집한 내용을
응답 문장에 바로 요약해 넣습니다.
...
| 응답 중단 | 자비스가 응답을 읽는 중 박수 두 번 → 즉시 중단, 재웨이크워드 없이 바로 다음 명령 대기 |
...
- **오디오 레벨 시각화**: 웹 대시보드(`JarvisFull`)가 브라우저 마이크 입력을 직접 캡처해 실시간 파형으로 표시 (백엔드 왕복 없음)
...
│   ├── web_collector.py            # ★ Playwright 기반 웹 수집 엔진 (2.0 신규)
...
│   ├── schedule.json
│   ├── bus_config.json
│   ├── web_collector_state.json    # 웹 수집 로그인 세션 (Playwright storage_state)
...
python -m tests.test_tts_interrupt              # 4-A: 박수 2번 → TTS 중단
python -m tests.test_skill_virtual_keyboard     # 4-B: 가상 키보드
python -m tests.test_listen_state               # 4-C: 음성 루프 상태 전환
python -m tests.test_claude_cli_engine_resume   # 4-D: run_task() --resume
```
