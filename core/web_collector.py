...
되돌릴 수 없는 액션은 클릭·입력 대상 요소의 텍스트에 그런 키워드가 보이면 최선의 노력으로
차단한다. 이는 텍스트 라벨 매칭에 불과해 완벽한 보장이 아니다 — "다음"/"확인"/"동의"처럼
다르게 표기되었거나 아이콘만 있는 컨트롤은 이 차단을 통과할 수 있다.
"""
...
# 클릭/입력 대상 요소 텍스트에 이 키워드가 포함되면 거부한다 — 이 엔진은
# 읽기 전용(검색/수집/필터링)만 다루고, 되돌릴 수 없는 액션은 모델 판단만으로
# 막을 수 없어(hybrid_screen.py의 잠금화면 클릭 사고 사례 참고) 최선의 노력으로
# 차단한다. 텍스트 라벨 매칭일 뿐이라 완벽한 보장은 아니다 — 다르게 표기되었거나
# 아이콘만 있는 컨트롤은 여전히 통과할 수 있다.
...
class WebCollectorEngine:
    ...
    def _collect_elements(self, page) -> list[WebElement]:
        ...
        """
        try:
            raw = page.evaluate(_COLLECT_JS.replace("__MAX_ELEMENTS__", str(_MAX_ELEMENTS)))
